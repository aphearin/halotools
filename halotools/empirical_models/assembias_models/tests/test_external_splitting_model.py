...
def test_splitting_model2():
    ...
    model.param_dict['mean_occupation_satellites_assembias_param1'] = 0.75
    ...
    all_cen_mask = model.mock.galaxy_table['gal_type'] == 'centrals'
    total_num_cens_all = np.count_nonzero(all_cen_mask)
    total_num_sats_all = np.count_nonzero(~all_cen_mask)
    print("\nTotal number of centrals in entire mock = {0}".format(total_num_cens_all))
    print("\nTotal number of satellites in entire mock = {0}".format(total_num_sats_all))
    print("Total number of centrals in mass bin = {0}\n".format(total_num_cens))
    print("Total number of satellites in mass bin = {0}\n".format(total_num_sats))
